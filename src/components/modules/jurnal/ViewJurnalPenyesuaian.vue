  ...
  <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      ...
      <div class="px-6 py-4 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900">Detail Jurnal Penyesuaian</h3>
          ...
          <div class="bg-gray-50 p-4 rounded-lg">
            ...
            <div class="space-y-3">
              <div class="flex justify-between">
                ...
                <span class="text-lg font-bold text-gray-900">Rp {{ formatNumber(displayJurnal.total_debit) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-sm text-gray-600">Total Kredit:</span>
                <span class="text-lg font-bold text-gray-900">Rp {{ formatNumber(displayJurnal.total_kredit) }}</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr v-else v-for="(detail, index) in displayJurnal.details" :key="detail.id">
                  ...
                  <td class="px-4 py-2 text-sm text-right font-medium text-gray-900">
                    Rp {{ formatNumber(detail.debit) }}
                  </td>
                  <td class="px-4 py-2 text-sm text-right font-medium text-gray-900">
                    Rp {{ formatNumber(detail.kredit) }}
                  </td>
                </tr>
...
const fetchFreshJurnalData = async (id) => {
  try {
    console.log(`Fetching fresh jurnal penyesuaian data for ID: ${id}`)
    loading.value = true

    const response = await fetch(`http://139.162.41.197:8001/api/jp/getJurnalPenyesuaian/${id}`)
    ...
    if (result.success && result.data) {
      ...
      console.error('Failed to fetch fresh jurnal penyesuaian data:', result.message)
      freshJurnalData.value = null
    }
  } catch (error) {
    console.error('Error fetching fresh jurnal penyesuaian data:', error)
    freshJurnalData.value = null
  } finally {
    loading.value = false
  }
...
watch(() => props.showModal, async (show) => {
  if (show && props.jurnal?.id) {
    console.log('Modal opened, fetching fresh data for jurnal penyesuaian ID:', props.jurnal.id)
...
const handleClose = () => {
  console.log('Close button clicked in ViewJurnalPenyesuaian')
