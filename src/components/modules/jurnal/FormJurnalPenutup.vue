    ...
    <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      ...
      <div class="px-6 py-4 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900">
          {{ isEdit ? 'Edit Jurnal Penutup' : 'Tambah Jurnal Penutup' }}
        </h3>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.log('Submitting jurnal penutup data:', submitData)

    let result
    if (isEdit.value) {
      result = await api.request(`/editJurnalPenutup/${formData.value.id}`, {
        method: 'PUT',
        body: JSON.stringify(submitData)
      }, 'jpa')
    } else {
      result = await api.request('/createJurnalPenutup', {
        method: 'POST', body: JSON.stringify(submitData)
      }, 'jpa')
    }
    console.log('Jurnal Penutup API Response:', result)
    ...
    if (result.success) {
      ...
      alert('Gagal menyimpan jurnal penutup: ' + (result.message || 'Unknown error'))
    }
  } catch (error) {
    console.error('Error saving jurnal penutup:', error)
    alert('Gagal menyimpan jurnal penutup: ' + error.message)
  } finally {
    loading.value = false
  }
}
...
const fetchJurnalData = async (id) => {
  try {
    console.log(`Fetching jurnal penutup data for ID: ${id}`)
    const result = await api.request(`/getJurnalPenutup/${id}`, {
      method: 'GET'
    }, 'jpa')
