  ...
  <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      ...
      <div class="px-6 py-4 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900">Detail Jurnal Penutup</h3>
...
const fetchFreshJurnalData = async (id) => {
  try {
    console.log(`Fetching fresh jurnal penutup data for ID: ${id}`)
    loading.value = true

    const response = await fetch(`http://139.162.41.197:8001/api/jpa/getJurnalPenutup/${id}`)
    ...
    if (result.success && result.data) {
      ...
      console.error('Failed to fetch fresh jurnal penutup data:', result.message)
      freshJurnalData.value = null
    }
  } catch (error) {
    console.error('Error fetching fresh jurnal penutup data:', error)
    freshJurnalData.value = null
  } finally {
    loading.value = false
  }
...
watch(() => props.showModal, async (show) => {
  if (show && props.jurnal?.id) {
    console.log('Modal opened, fetching fresh data for jurnal penutup ID:', props.jurnal.id)
...
const handleClose = () => {
  console.log('Close button clicked in ViewJurnalPenutup')
