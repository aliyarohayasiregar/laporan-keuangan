  ...
  <div class="min-h-screen bg-gray-100 flex">
    ...
    <aside :class="[
      ...
      <nav class="px-4 pb-6 flex-1">
        <div class="space-y-1">
          ...
          <router-link v-if="hasPermission('pengaturan akun')" to="/pengaturan-akun" :class="[
            'w-full text-left px-4 py-3 rounded-lg font-medium transition-colors duration-200 flex items-center gap-3',
            route.name === 'pengaturan-akun'
              ? 'bg-blue-600 text-white'
              : 'text-gray-700 hover:bg-gray-100'
          ]">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <main class="flex-1 lg:ml-64 ml-0 h-screen overflow-auto">
      <div class="p-4 lg:p-6 pt-16 lg:pt-6">
        ...
        <!-- Pengaturan Akun Page -->
        <div v-else-if="route.name === 'pengaturan-akun'">
          <PengaturanAkun />
        </div>
...
import PengaturanAkun from '../pengaturanAkun/PengaturanAkun.vue'
...
const handleDelete = async (item) => {
  const ok = await showConfirm({
    type: 'danger',
    title: 'Hapus Barang',
    message: `Apakah Anda yakin ingin menghapus <${item.nama_barang}? Data yang dihapus tidak dapat dikembalikan.`,
